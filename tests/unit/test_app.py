...
def setup(mock_db, mock_fs):
  with patch("imaginate_api.app.db", mock_db), patch("imaginate_api.app.fs", mock_fs):
    yield
...
@pytest.mark.parametrize(
  "_id, expected",
  [("621f1d71aec9313aa2b9074cd", BadRequest), ("", BadRequest), (None, BadRequest)],
)
...
@pytest.fixture
def mock_mongo_functions():
  with (
    patch("app.validate_id") as mock_validate_id,
    patch("app.search_id") as mock_search_id,
  ):
    yield mock_validate_id, mock_search_id
